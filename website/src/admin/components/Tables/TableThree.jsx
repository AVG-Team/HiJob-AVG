...
export default function TableThree () {
    const renderPackageRows = () => {
        return packageData.map((packageItem, key) => (
            <PackageRow key={key} packageItem={packageItem} />
        ));
    };
...
const PackageRow: React.FC<PackageRowProps> = ({ packageItem }) => {
    ...
    return (
        <tr>
            ...
            <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${getStatusColor()}`}>
                    {packageItem.status}
                </p>
            </td>
            <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
